...
#!!!
			...
			if add2dic:
				...
				valid_triple[(t, r + rel_n, h)] = True
				if (h, r) not in dic:
					dic[(h, r)] = []
				dic[(h, r)].append(t)
				if (t, r + rel_n) not in dic:
					dic[(t, r + rel_n)] = []
				dic[(t, r + rel_n)].append(h)
...
# load data
reader = reader("data/")
tra_d = reader.load("train.txt")
val_d = reader.load("valid.txt")
tes_d = reader.load("test.txt")
...
def tensor_constant(value, shape):
	return torch.ones(shape, device = device) * value
...
class Model(torch.nn.Module):
	def __init__(self, ent_n, rel_n, dim, p = 2):
		...
		self.rel_emb = self._init_emb(rel_n * 3)
	...
	# This function is used to replaced the default L2 normalization, which works slow
	#@staticmethod
		
	def transform(self, h_ids, r_typ):
		r_ids = r_typ * 3

		h = self.ent_emb(h_ids)
		r = self.rel_emb(r_ids)
		w_r = self.rel_emb(r_ids + 1)
		# r_r = self.rel_emb(r_ids + 2)

		t = h - torch.sum(h * w_r, dim = 1).unsqueeze(1) * w_r + r

		# t += torch.randn(t.size(), device = device) / 4.0
	...
	@staticmethod
	def to_hyperplane(v, n):
		ones = tensor_constant(1, (v.shape[1], v.shape[1]))
		
		#print(v * n, type(v * n))
		#print(ones, type(ones))
		
		return v - torch.matmul(v * n, ones) * n

	def diff(self, v1, v2, d = -1):
		return (v1 - v2).norm(dim = d, p = self.p)**2
	...
	def forward(self, h_ids, r_typ, t_ids):
		...
		r = self.rel_emb(r_ids)
		n_r = self.rel_emb(r_ids + 1)
		
		F.normalize(n_r)

		h = self.to_hyperplane(h, n_r)
		t = self.to_hyperplane(t, n_r)
		
		F.normalize(h)
		F.normalize(r)
		F.normalize(t)

		# lhs = self.transform(h_ids, r_typ)
		# rhs = t - torch.sum(t * w_r, dim = 1).unsqueeze(1) * w_r
	...
	def loss(self, h_ids, r_typ, t_ids):
		...
		if (dirc == 1):
			L += F.margin_ranking_loss(pos, neg, target = -torch.ones_like(pos), margin = 1.)
		else:
			L += F.margin_ranking_loss(neg, pos, target = -torch.ones_like(pos), margin = 1.)
		
		return L

		E = self.ent_emb.weight.data
		
		r_typ = torch.arange(rel_n, device = device)
		W = self.rel_emb(r_typ * 3 + 1)
		R = self.rel_emb(r_typ * 3)

		L1 = torch.relu(E.norm(dim = 1)**2 - 1)
		L2 = torch.relu(torch.sum(R * W, dim = 1)**2 / R.norm()**2)
		
		C = 0.015625
		L += C * (torch.sum(L1) + torch.sum(L2))

		# return torch.sum(pos) - torch.sum(neg)
		# return torch.sum(pos)
		# return torch.sum(pos) + torch.sum(torch.abs(neg))# + torch.sum(gap)
		
		# criterion = torch.nn.MarginRankingLoss(margin=1.0, reduction='mean')
		# return criterion(pos, neg, -torch.ones_like(pos))
...
def eval_batch_wise(h_ids, r_typ, t_ids):
	# res: [batch_s, dim] -> [batch_s, 1, dim] -> [batch_s, ent_n, dim]
	lhs = model.transform(h_ids, r_typ)
	lhs = torch.unsqueeze(lhs, dim = 1)
	lhs = lhs.expand(lhs.shape[0], model.ent_n, model.dim)

	# embed_tail: [batch_s, emb_n] -> [batch_s, ent_n, emb_n]
	t = model.ent_emb.weight.data
	r_r = model.rel_emb(3 * r_typ + 2)
	# t *= r_r
	# print(t, t.size())
	# print(r_r, r_r.size())
	rhs = t.expand(lhs.shape[0], model.ent_n, model.dim)

	# values: [batch size, k] scores, the smaller, the better
	# indices: [batch size, k] indices of entities ranked by scores
	score = dirc * model.diff(lhs, rhs, 2)
...
def eval_with_filter(data, colo = "green", batch_s = 2000):
	...
	for i in tqdm(range(n), colour = colo):
		...
		score = debuff + dirc * model.forward(h.expand_as(ents), r.expand_as(ents), ents)
...
model = Model(ent_n, rel_n * 2, dim, 2)
model.to(device)
