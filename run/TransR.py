...
class Model(torch.nn.Module):
	def __init__(self, ent_n, rel_n, dim, p = 2):
		...
		#self.mat_emb = self._init_emb(rel_n * dim)
	...
	def forward(self, h_ids, r_typ, t_ids):
		...
		off = torch.arange(dim, device = device).repeat(r_typ.numel())
		r_ids = (r_typ * dim).repeat_interleave(dim) + off
		
		M_r = self.mat_emb(r_ids)
		M_r = M_r.view(-1, dim, dim)
		
		h = self.to_r_space(h, M_r)
		t = self.to_r_space(t, M_r)
