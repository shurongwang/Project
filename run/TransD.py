...
class Model(torch.nn.Module):
	def __init__(self, ent_n, rel_n, dim, p = 2):
		...
		self.mt1_emb = self._init_emb(rel_n)
		self.mt2_emb = self._init_emb(ent_n)
	...
	def forward(self, h_ids, r_typ, t_ids):
		...
		r_p = self.mt1_emb(r_p)
		h_p = self.mt2_emb(h_ids)
		t_p = self.mt2_emb(t_ids)
		
		r_p = r_p.unsqueeze(-1)
		M_rh = r_p * h_p.unsqueeze(-1).transpose(1, 2)
		M_rt = r_p * h_t.unsqueeze(-1).transpose(1, 2)
		
		h = self.to_space(h, M_rh)
		t = self.to_space(t, M_rt)
...
@torch.no_grad()
def eval(data, colo = "green", batch_s = 25):
	model.eval()
...
@torch.no_grad()
def eval_with_filter(data, colo = "green", batch_s = 1000):
	model.eval()
